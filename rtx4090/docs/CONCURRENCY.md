# Concurrency & `--parallel` Tuning (RTX 4090, 24GB)

> Measured 2026-08 on the original 4090 build (git `main`). Same GPU model as
> the new box; not yet re-run there.
...
Current setting: `rtx4090/scripts/start-llama-35b-moe.sh` passes `--parallel 2` (48 K per slot).
...
- More slots also **shrink per-slot KV cache**: `--ctx-size / --parallel`. With `--ctx-size 98304`:
  - `--parallel 2` → 48 K per slot (current)
  - `--parallel 4` → 24 K per slot
  - `--parallel 8` → 12 K per slot
...
1. Edit `rtx4090/scripts/start-llama-35b-moe.sh` (`--parallel N`).
...
The default is `--parallel 2`: single-user speed, 48 K per slot, and room for one background agent alongside an interactive session. `--parallel 2` was not in the sweep below; interpolate between 1 and 4.
...
| Multi-user up to ~8 concurrent | `--parallel 8` | Best aggregate at N=2/4/10, 12 K per-slot context still usable for chat |
