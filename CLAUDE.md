...
- **Send `chat_template_kwargs: {"reasoning_effort": "medium"}`** (or `low`) with requests — the chat-template default is `xhigh`, which burns ~10× tokens and ~10–12× wall time for no measured pass-rate gain on coding tasks (2026-08-16 sweep, `benchmarks/bench_effort.py`, results in BENCHMARKS.md). Thinking fully off is measurably weaker. Vision (base64 `image_url`) and tool calling both work on the production config as-is.
