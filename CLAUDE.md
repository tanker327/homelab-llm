...
Local LLM inference server on an RTX PRO 6000 Blackwell (96GB VRAM, SM120, CUDA 13.1; originally built for an RTX 4090 — comments in some launchers note the old 24GB settings), exposing an OpenAI-compatible API on port 5000. The systemd default since 2026-08-28 is **llama.cpp serving `Qwen3.8-Flash-Next-Uncensored` IQ4_XS** (orcarouter abliterated 177B MoE, 6B active/token) via `scripts/start-production.sh`, which execs whichever launcher `scripts/production-engine.conf` names (currently `start-llama-flashnext.sh`): full 262K native context, vision on (mmproj), ~75-76GB VRAM — the model's 51B PLE n-gram table is mmap'd host-side, not in VRAM. ~109 tok/s single-stream (no MTP head exists in the GGUF), ~3000 tok/s prefill; **aggregate throughput plateaus ~130 tok/s at N=4 — for agent-fleet loads switch the conf line back to `start-vllm-38-27b-nvfp4.sh` (vLLM `Qwen3.8-27B-NVFP4-Inferact`, the 2026-08-16→28 default: MTP n=3 + FP8 KV, ~676 agg @ N=16, ~129 tok/s single-stream, text-only) and `sudo systemctl restart llama-server`.** Cold-from-disk service start can take ~10-25 min before `/health` is up (~20s warm). `start-vllm-38-27b-fp8.sh` (vision-capable 27B) and `scripts/start-vllm-27b-fp8.sh` (Qwen3.6, 2026-08-12 bake-off winner) remain as rollbacks; see the 2026-08-28 addendum in `docs/BENCHMARKS.md`. The former default two-model llama.cpp **agent stack** (`start-agent-stack.sh`: 35B MoE workers on 5000 + 27B Q8_0 on 5001) and all llama.cpp launchers remain for manual use. System-level changes (systemd, firewall, host config) are tracked in `CHANGELOG.md`.
...
./scripts/start-production.sh            # SYSTEMD DEFAULT: runs the launcher named in production-engine.conf (currently Flash-Next)
./scripts/start-llama-flashnext.sh       # Production engine: llama.cpp + Flash-Next-Uncensored 177B IQ4_XS (262K ctx, vision, ~76GB, ~109 tok/s)
./scripts/start-vllm-38-27b-nvfp4.sh     # Agent-fleet engine: vLLM + Qwen3.8-27B NVFP4 + MTP n=3 + FP8 KV (262K, 16 seqs, ~88GB, TEXT-ONLY)
...
Only one engine can bind port 5000 at a time — and the Flash-Next default (~76GB) plus any second model won't fit in VRAM anyway. Stop the service, wait ~10s for VRAM release, then launch a manual engine.
...
- **Reasoning effort**: the Flash-Next production template's default is moderate (no server-side override set; per-request `chat_template_kwargs: {"reasoning_effort":"low"}` clamps it, `"minimal"` is invalid). The three Qwen3.8 **vLLM** launchers default to `low` since 2026-08-26: they pass `--default-chat-template-kwargs '{"reasoning_effort":"low"}'`, overriding the chat template's `xhigh` default (which burns ~10× tokens and ~10–12× wall time for no measured pass-rate gain on coding tasks — 2026-08-16 sweep, `benchmarks/bench_effort.py`, results in BENCHMARKS.md). Per-request `chat_template_kwargs: {"reasoning_effort": ...}` still overrides it (vLLM merges request kwargs over the default). Thinking fully off is measurably weaker. Vision (base64 `image_url`) and tool calling both work on the production config as-is.
...
- Cold-start times: Flash-Next (production) ~20s–1 min with warm page cache, ~10–25 min cold from disk; small llama.cpp GGUFs ~5s; vLLM ~2–3 min (weight load + JIT + CUDA graphs); SGLang ~2–3 min.
